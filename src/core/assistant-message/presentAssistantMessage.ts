...
import { executeDddTool, isDddTool } from "../tools/dddToolExecutor"
		...
		case "tool_use":
			const toolDescription = (): string => {
				switch (block.name) {
					...
					case "ddd_init_layer":
						return `[${block.name} for '${(block.params as any).layer || "unknown"}' layer]`
			...
			switch (block.name) {
				...
				case "attempt_completion":
					...
					break
				default:
					// Check if it's a DDD tool
					if (isDddTool(block.name)) {
						await executeDddTool(cline, block, askApproval, handleError, pushToolResult, removeClosingTag)
						break
					}
